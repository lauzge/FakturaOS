...
uses
  ...
  ExtCtrls, Math,
  VpBase, VpBaseDS, VpMisc,
  VpData, VpSr, VpConst,
  VpCanvasUtils, VpEvntEditDlg;
...
type
  ...
  published
    ...
    property Color: TColor read FColor write SetColor default clBtnFace;
    property EventBackgroundColor: TColor read FEventBackgroundColor write SetEventBackgroundColor default clWindow;
    property Font: TVpFont read FFont write SetFont;
  end;
  ...
  published
    property AllDayEventAttr: TVpERAllDayEventAttributes read FAllDayEventAttr write FAllDayEventAttr;
    ...
    property ReportAttr: TVpERReportAttributes read FReportAttr write FReportAttr;
    property SeparateEvents: TVpEREventSeparator read FSeparateEvents write SetSeparateEvents default esLine;
    property ShowCategoryColors: boolean read FShowCategoryColors write SetShowCategoryColors default True;
    property ShowEventTime: boolean read FShowEventTime write SetShowEventTime default True;
    property ShowResourceName: boolean read FShowResourceName write SetShowResourceName default True;
    property TimeFormat: TVpTimeFormat read FTimeFormat write SetTimeFormat default tf24Hour;
...
begin
  if v <> FEventBackgroundColor then
  begin
...
begin
  ...
  begin
    ...
    if Verify then
      DoIt := (MessageDlg(RSDelete + ' ' + Str + ' ' + RSFromSchedule + #13#10#10 + RSPermanent, mtConfirmation,
        [mbYes, mbNo], 0) = mrYes);
    ...
    begin
      ...
      if (ShowCategoryColors) and (not FActiveEvent.AllDayEvent) then
        ...
        erInPlaceEditor.Move(Rect(erActiveEventRec.Left + TextMargin + VpColorCategoryWidth,
          erActiveEventRec.Top + 2, erActiveEventRec.Right -
          TextMargin, erActiveEventRec.Bottom - 2),
          True)
      else
        erInPlaceEditor.Move(Rect(erActiveEventRec.Left + TextMargin, erActiveEventRec.Top +
          2, erActiveEventRec.Right - TextMargin, erActiveEventRec.Bottom - 2),
          True);
...
begin
  ...
  begin
    if erInPlaceEditor.Text <> FActiveEvent.Description then
    begin
...
begin
  ...
  begin
    if erEventArray[I].Event = nil then
    begin
    ...
    if (Pt.X > erEventArray[I].Rec.Left) and (Pt.X < erEventArray[I].Rec.Right) and (Pt.Y > erEventArray[I].Rec.Top) and
      (Pt.Y < erEventArray[I].Rec.Bottom) then
    begin
...
begin
  ...
  Idx := -1;
  for i := 0 to Pred(Length(erEventArray)) do
    if erEventArray[i].Event = AnEvent then
    begin
      Idx := i;
      Break;
    end;
  ...
  for i := Idx + 1 to Pred(Length(erEventArray)) do
    if Assigned(erEventArray[i].Event) then
    begin
...
begin
  ...
  begin
    ...
    if RSDayPopupNavYesterday <> '' then
    begin
      NewSubItem := TMenuItem.Create(Self);
    ...
    if RSDayPopupNavNextMonth <> '' then
    begin
      NewSubItem := TMenuItem.Create(Self);
    ...
    if RSDayPopupNavPrevMonth <> '' then
    begin
      NewSubItem := TMenuItem.Create(Self);
...
procedure TVpEventReport.LinkHandler(Sender: TComponent; NotificationType: TVpNotificationType; const Value: variant);
begin
...
begin
  case Key of
    ...
    VK_F10:
      if (ssShift in Shift) and not (Assigned(PopupMenu)) then
      begin
...
begin
  ...
  StartTime := trunc(FDisplayDate) + EncodeTime(12, 0, 0, 0);
  EndTime := StartTime + EncodeTime(13, 0, 0, 0);
  ;
  FActiveEvent := DataStore.Resource.Schedule.AddEvent(DataStore.GetNextID(EventsTableName),
    StartTime, EndTime);
...
procedure TVpEventReport.RenderToCanvas(RenderCanvas: TCanvas; RenderIn: TRect;
  Angle: TVpRotationAngle; Scale: extended;
  RenderDate: TDateTime; StartLine: integer;
  StopLine: integer; UseGran: TVpGranularity;
  DisplayOnly: boolean);
...
var
  ...
  begin
    ...
    begin
      ...
      DrawBevelRect(RenderCanvas,
        TPSRotateRectangle(Angle, RenderIn, Rect(RealLeft,
        RealTop, RealRight - 1, RealBottom - 1)),
        BevelShadowColor,
        BevelHighlightColor);
      DrawBevelRect(RenderCanvas,
        TPSRotateRectangle(Angle, RenderIn, Rect(RealLeft +
        1, RealTop + 1, RealRight - 2, RealBottom - 2)),
        BevelShadowColor,
        BevelHighlightColor);
    end
    else if FDrawingStyle = ds3d then
    begin
      { draw a 3d bevel }
      DrawBevelRect(RenderCanvas,
        TPSRotateRectangle(Angle, RenderIn, Rect(RealLeft,
  ...
  begin
    ...
    try
      ...
      ColHeadRect.Bottom := ColHeadRect.Top + ColRowHeight + TextMargin * 2 + 2;
      ...
      TPSFillRect(RenderCanvas, Angle, RenderIn,
        Rect(ColHeadRect.Left + 2, ColHeadRect.Top + 2, ColHeadRect.Right -
        2, ColHeadRect.Bottom));

      ResStr := FormatDateTime(FDateLabelFormat, RenderDate);
      ResStrLen := RenderCanvas.TextWidth(ResStr);
      if ResStrLen > ColHeadRect.Right - ColHeadRect.Left then
      begin
        ResStr := GetDisplayString(RenderCanvas, ResStr, 0, ColHeadRect.Right - ColHeadRect.Left);
      end;
      ...
      if (DataStore <> nil) and (DataStore.Resource <> nil) and FShowResourceName then
      begin
        { fix Res String }
...
{          HeadRect.Left := RealLeft + 1;
          ...
          DrawBevelRect(RenderCanvas,
            ...
            ColHeadRect.Left + 2, ColHeadRect.Top + 2,
            ColHeadRect.Right - 2, ColHeadRect.Bottom)),
...
{          HeadRect.Left := RealLeft + 2;
          ...
          DrawBevelRect(RenderCanvas,
            TPSRotateRectangle(Angle, RenderIn, Rect(
            ColHeadRect.Left + 3, ColHeadRect.Top + 3,
            ColHeadRect.Right - 4, ColHeadRect.Bottom - 1)),
  ...
  begin
    ...
    try
      ...
      try
        ...
        begin
          if AdEventsList.Count > 0 then
          begin
            for K := 0 to pred(AdEventsList.Count) do
            begin
              if TVpEvent(AdEventsList[K]) = TVpEvent(TempList[J]) then
              begin
                Skip := True;
                Break;
              end;
      ...
      begin
        ...
        if AdEventsRect.Top + (MaxADEvents * ADTextHeight) + TextMargin * 2 > DayRect.Bottom then
        ...
        for I := 0 to pred(ADEventsList.Count) do
        begin
          ...
          if ADEventsRect.Top + TextMargin + ((I + 1) * ADTextHeight) - TextMargin > DayRect.Bottom then
          begin
          ...
          TPSTextOut(RenderCanvas, Angle, RenderIn,
            ...
            EventStr);
          Result := True;
          erEventArray[EAIndex].Rec := Rect(ADEventRect.Left + TextMargin, ADEventRect.Top +
            TextMargin, ADEventRect.Right - TextMargin,
            ADEventRect.Bottom);
  ...
  begin
    ...
    try
      ...
      TextRect.TopLeft := Point(ColHeadRect.Left, ColHeadRect.Bottom);
      TextRect.BottomRight := Point(ColHeadRect.Right, TextRect.Top + RealRowHeight);

      { Handle All Day Events }
      if DrawAllDayEvents(ADate, Rect(TextRect.Left + 2, TextRect.Top +
        1, TextRect.Right - 2, RealBottom), EAIndex) then
      begin
      ...
      for j := 0 to pred(EventList.Count) do
      begin
        ...
        if TextRect.Bottom - TextMargin > RealBottom then
        begin
          RenderCanvas.Brush.Color := RealDotDotDotColor;
          { draw dot dot dot }
          TPSFillRect(RenderCanvas, Angle, RenderIn,
            Rect(TextRect.Right - 20, TextRect.Bottom - 7, TextRect.Right - 17, TextRect.Bottom - 4));
          TPSFillRect(RenderCanvas, Angle, RenderIn,
            Rect(TextRect.Right - 13, TextRect.Bottom - 7, TextRect.Right - 10, TextRect.Bottom - 4));
          TPSFillRect(RenderCanvas, Angle, RenderIn,
            Rect(TextRect.Right - 6, TextRect.Bottom - 7, TextRect.Right - 3, TextRect.Bottom - 4));
          break;
        end;
        ...
        begin
          if TimeFormat = tf24Hour then
            EventStr := FormatDateTime('hh:nn', TVpEvent(EventList.List^[j]).StartTime) +
              ' - ' + FormatDateTime('hh:nn', TVpEvent(EventList.List^[j]).EndTime) + ': '
          else
            EventStr := FormatDateTime('hh:nn AM/PM', TVpEvent(EventList.List^[j]).StartTime) +
        ...
        if EventStr = '' then
          ...
          EventStr := EventStr + ' ' + TVpEvent(EventList.List^[j]).Description;
        ...
        begin
          ...
          begin
            EventStr := GetDisplayString(RenderCanvas, EventStr,
              0, TextRect.Right - TextRect.Left -
              (TextMargin * 2));
          end;
          RenderCanvas.Brush.Color := GetCategoryBackgroundColor(TVpEvent(EventList.List^[j]));
          TPSFillRect(RenderCanvas, Angle, RenderIn,
            Rect(TextRect.Left + 2, TextRect.Top + 1, TextRect.Right - 2, TextRect.Bottom - 1));
        ...
        begin
          ...
          while (not TextDone) and (TextRect.Top < RealBottom) do
          begin
            RenderCanvas.Brush.Color := GetCategoryBackgroundColor(TVpEvent(EventList.List^[j]));
            TPSFillRect(RenderCanvas, Angle, RenderIn,
              Rect(TextRect.Left + 2, TextRect.Top + 3, TextRect.Right -
              2, TextRect.Bottom + TextMargin * 2 - 1));
            i := RenderTextToRect(RenderCanvas, Angle, RenderIn, Rect(TextRect.Left +
              TextMargin div 2 + TextOffset, TextRect.Top, TextRect.Right,
              TextRect.Bottom + TextMargin * 2), EventStr);
            if i < TextLen then
            begin
        ...
        if FActiveEvent = TVpEvent(EventList.List^[j]) then
        begin
          TPSDrawFocusRect(RenderCanvas, Angle, RenderIn,
            Rect(TextRect.Left + 2 + TextOffset, TextRect.Top +
            1, TextRect.Right - 2, TextRect.Bottom - 1));
        end;

        if FShowCategoryColors then
        begin
          RenderCanvas.Brush.Color := DataStore.CategoryColorMap.GetColor(TVpEvent(EventList.List^[j]).Category);
          TPSFillRect(RenderCanvas, Angle, RenderIn,
            Rect(TextRect.Left + 2, TextRect.Top + 3, TextRect.Left +
            TextOffset, TextRect.Bottom - 2));
        end;
...
procedure TVpEventReport.WMLButtonDBLCLK(var Message: TLMLButtonDblClk);
var
...
begin
  ...
  if (Message.YPos > erHeaderHeight) then
  begin
    ...
    else if (DataStore.Resource <> nil) then
    begin
      ...
      FActiveEvent := DataStore.Resource.Schedule.AddEvent(DataStore.GetNextID('Events'), StartTime, EndTime);
      { edit this new event }
      erSpawnEventEditDialog(True);
    end;
  end;
...
procedure TVpEventReport.WMLButtonDown(var Message: TLMLButtonDown);
begin
  inherited;
  ...
  begin
    ...
    if EventAtCoord(Point(Message.XPos, Message.YPos)) then
      erClickTimer.Enabled := True;
  end;
  Invalidate;
end;

procedure TVpEventReport.WMRButtonDown(var Message: TLMRButtonDown);
var
  ClientOrigin: TPoint;
  i: integer;
...
begin
  ...
  begin
    if not focused then
      SetFocus;
    { The mouse click landed inside the client area }
    EventAtCoord(Point(Message.XPos, Message.YPos));
    erClickTimer.Enabled := False;
    ...
    FDefaultPopup.Popup(Message.XPos + ClientOrigin.x,
      Message.YPos + ClientOrigin.y);
  end;
end;
