...
uses
  Windows, Classes, Dialogs, SysUtils, Db, ZCompatibility,
  ZConnection, ZDataset, VpBase, VpData, VpSR, VpBaseDS,
  VpDBDS, VpException, VpConst, LResources;
...
begin
  ...
  FEventsTable.ObjectView:=true; // von Volker geändert .AlternativeView :=True;
...
//  FEventsTable.RequestLive := True;
  ...
  FContactsTable.ObjectView:=true; // von Volker geändert .AlternativeView :=True;
...
//  FContactsTable.RequestLive := True;
  ...
  FTasksTable.ObjectView:=true; // von Volker geändert .AlternativeView :=True;
...
//------------------------------------------------------------------------------
destructor TVpCaoDataStore.Destroy;
begin
  { free tables }
...
//------------------------------------------------------------------------------
function TVpCaoDataStore.GetConnected: Boolean;begin
...
//------------------------------------------------------------------------------
procedure TVpCaoDataStore.Load;
var Res : TVpResource;
begin
...
//------------------------------------------------------------------------------
procedure TVpCaoDataStore.CreateTable(TableName: string);
var
  Table: TZTable;
begin
  ...
  try
    ...
    Table.ObjectView:=true; // von Volker geändert .AlternativeView :=True;
...
//------------------------------------------------------------------------------
procedure TVpCaoDataStore.PostResources;
var
  I: Integer;
...
//------------------------------------------------------------------------------
procedure TVpCaoDataStore.PostEvents;
var
  I: Integer;
  Event: TVpEvent;
  Qry: TZQuery;
...
//      Qry.RequestLive     := true;
      Qry.ObjectView:=true; // von Volker geändert .AlternativeView := True;
...
//------------------------------------------------------------------------------
procedure TVpCaoDataStore.PostContacts;
var
  I: Integer;
  Contact: TVpContact;
  Qry: TZQuery;
...
//------------------------------------------------------------------------------
procedure TVpCaoDataStore.PostTasks;
var
  I: Integer;
  Task: TVpTask;
  Qry : TZQuery;
begin
  ...
  begin
    ...
    try
      Qry.Connection := FConnection;
      Qry.ObjectView:=true;  // volker
//      Qry.RequestLive := true;
//      Qry.AlternativeView :=true;
...
//------------------------------------------------------------------------------
procedure TVpCaoDataStore.PurgeResource(Res: TVpResource);
begin
  Res.Deleted := true;
  PostResources;
  Load;
...
//------------------------------------------------------------------------------
procedure TVpCaoDataStore.PurgeEvents(Res: TVpResource);
var
  Qry: TZQuery;
begin
  Qry := TZQuery.Create(self);
...
//------------------------------------------------------------------------------
procedure TVpCaoDataStore.PurgeContacts(Res: TVpResource);
var
  Qry: TZQuery;
begin
  Qry := TZQuery.Create(self);
...
//------------------------------------------------------------------------------
procedure TVpCaoDataStore.PurgeTasks(Res: TVpResource);
var
  Qry: TZQuery;
begin
  Qry := TZQuery.Create(self);
...
//------------------------------------------------------------------------------
procedure TVpCaoDataStore.SetConnected(const Value: boolean);
var
  DBPath : string;
  Str    : string;
begin
  ...
  // Open Contacts Table
  FContactsTable.Connection :=FConnection;
  FContactsTable.ObjectView :=True;
  ...
  inherited SetConnected(Connection.Connected);
end;
//------------------------------------------------------------------------------
...
procedure TVpCaoDataStore.SetFilterCriteria(aTable :TDataset;
...
//------------------------------------------------------------------------------
procedure TVpCaoDataStore.CreateIndexDefs(const TableName: string;
  IndexDefs: TIndexDefs);
begin
end;
//------------------------------------------------------------------------------
procedure TVpCaoDataStore.CreateFieldDefs(const TableName: string;
