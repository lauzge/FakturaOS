...
uses
  ...
  LResources, Buttons, ExtCtrls, StdCtrls, Menus, VolDBConst;
...
//    OnCanResize := MainCanResize;
  ...
  begin
    k := DaysPerMonth(FYear, FMonth); //÷èñëî äíåé â ìåñÿöå
    if FDay > k then FDay := k;
    FDate := EncodeDate(FYear, FMonth, FDay);
    LabelMon.Caption := LongMonthNames[FMonth];
    LabelYear.Caption := IntToStr(FYear);
    //îáíîâëåíèå áóòòîíîâ
    d1 := DayOfWeek(EncodeDate(FYear, FMonth, 1)); //äåíü íåäåëè 1-ãî äíÿ ìåñÿöà
    if FMondayWeek then
