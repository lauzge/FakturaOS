...
var Func1, Func2: TFilterRecordEvent;
  FL: TList;
begin
  if (FKindSearch = vksFindFirst) {or (ListLink.DataSet = nil)} then Exit;   // geändert Volker
  FSearchText := '';
  FL := TList.Create;
//  ListLink.DataSet.GetFieldList(FL, ListField);  von Volker geändert
  FViewField := TField(FL[ListFieldIndex]).FieldName;
  FL.Free;
//  Func1 := ListLink.DataSet.OnFilterRecord;   von Volker geändert
  Func2 := AFilterRecord;
...
//    ListLink.DataSet.Filtered := FSearchText > '';     geänder von Volker
  end;
end;
