...
{ Modification c. 2003 Jan Pokrandt / Jan@JP-Soft.de }

{*******************************************************************************

$Id: CAO_SearchClass.pas,v 1.5 2004/05/07 18:29:27 jan Exp $

CVS-Log :
$Log: CAO_SearchClass.pas,v $
Revision 1.5  2004/05/07 18:29:27  jan
kleine Bugfixes  (V. 1.2.5.3)

Revision 1.2  2003/09/20 11:22:44  jan
Bug bei Suche nach "and" oder "or" beseitigt
...
begin
  ...
  else if (FCurWord <> '') and (FCurWord <> ')') then begin
    Result.Append(spParseExpr);
    Result.Append(TaaRPN_AND.Create);
  end;
end;
...
begin
  ...
  if (FHead = 0) then
    ...
    ToInx := FQueue.Count;
    for FromInx := pred(OldCount) downto FHead do begin
      dec(ToInx);
