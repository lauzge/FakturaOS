...
uses {$IFNDEF LINUX}Windows{$ELSE}Types{$ENDIF}, SysUtils, Classes, ZTokenizer, Math;

{$INCLUDE C:\codetyphon\typhon\components\pl_ZeosDBO\source\Zeos.inc}
...
begin
  try
    P := TObject(Obj);
    //JAN
    if not assigned(TObject(Obj)) then exit;
    // JAN ENDE
